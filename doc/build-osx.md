...
====================================
This guide will show you how to build netrumd (headless client) for OSX.
...
1. Clone the github tree to get the source code and go into the directory.
        ...
        git clone https://github.com/netrumproject/Netrum.git
        cd Netrum
...
5.  (Optional) You can also install netrumd to your path:
...
4. Enter "netrum-qt" as project name, enter src/qt as location
...
------------------------
You can ignore this section if you are building `netrumd` for your own use.

netrumd/netrum-cli binaries are not included in the netrum-Qt.app bundle.

If you are building `netrumd` or `netrum-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the Netrum-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./netrumd`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./netrumd` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=netrumrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/Netrum/netrum.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/Netrum/netrum.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/Netrum/debug.log

Other commands:
-------

    ./netrumd -daemon # to start the netrum daemon.
    ./netrum-cli --help  # for a list of command-line options.
    ./netrum-cli help    # When the daemon is running, to get a list of RPC commands
...
* Database errors have been seen in builds on High Sierra. One solution is to build Berkeley DB from source.
        ...
        Then configure Netrum with this build of BerkeleyDB,
...
The use homebrew to install a number of unix programs and libraries needed to build the Netrum wallet:
...
You have the choice to build the GUI Netrum wallet as a Mac OSX app, described in “How to build the Netrum-Qt App”. If, for whatever reason, you prefer to use the command line tools, continue with “Command line tools”.

### How to build the Netrum-Qt App:

After make is finished, you can create an App bundle inside a disk image with:

```make deploy```

Once this is done, you’ll find Netrum-Qt.dmg inside your Netrum folder. Open and install the wallet like any typical Mac app.
...
~/Downloads/Netrum/src/qt/netrum-qt