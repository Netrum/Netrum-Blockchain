...
#if defined(NDEBUG)
#error "Netrum cannot be compiled without assertions."
#endif
...
{
    RenameThread("netrum-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateNTRSupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/Netrum Core:1.1.0/" ||
            pfrom->cleanSubVer == "/Netrum Core:1.3.0/" ||
            pfrom->cleanSubVer == "/Netrum Core:1.3.1/") {
            LOCK(cs_main);
        ...
        // Netrum: We use certain sporks during IBD, so check to see if they are
