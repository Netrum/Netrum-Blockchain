...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NTR);
    ...
    if (!settings.contains("nAnonymizeNetrumAmount"))
        settings.setValue("nAnonymizeNetrumAmount", 1000);

    nAnonymizeNetrumAmount = settings.value("nAnonymizeNetrumAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNetrumAmount"))
        SoftSetArg("-anonymizenetrumamount", settings.value("nAnonymizeNetrumAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in netrum.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNetrumAmount:
            return QVariant(nAnonymizeNetrumAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNetrumAmount:
            nAnonymizeNetrumAmount = value.toInt();
            settings.setValue("nAnonymizeNetrumAmount", nAnonymizeNetrumAmount);
            emit anonymizeNetrumAmountChanged(nAnonymizeNetrumAmount);
            break;
        case CoinControlFeatures:
