...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::NTR)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, QString& sNTRPercentage)
{
    int nPrecision = 2;
    ...
    sNTRPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    ui->labelNTRPercent->setText(sPercentage);
    ...
    bool showNTRAvailable = settingShowAllBalances || phrAvailableBalance != nTotalBalance;
    bool showWatchOnlyNTRAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showNTRPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyNTRPending = watchUnconfBalance != 0;
    bool showNTRLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyNTRLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showNTRAvailable || showWatchOnlyNTRAvailable);
    ui->labelBalanceText->setVisible(showNTRAvailable || showWatchOnlyNTRAvailable);
    ui->labelWatchAvailable->setVisible(showNTRAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showNTRPending || showWatchOnlyNTRPending);
    ui->labelPendingText->setVisible(showNTRPending || showWatchOnlyNTRPending);
    ui->labelWatchPending->setVisible(showNTRPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showNTRLocked || showWatchOnlyNTRLocked);
    ui->labelLockedBalanceText->setVisible(showNTRLocked || showWatchOnlyNTRLocked);
    ui->labelWatchLocked->setVisible(showNTRLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("NTR")
    updateDisplayUnit();
}
